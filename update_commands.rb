...
# Use this script to update the commands auto-completed in plugin/consul.vim.

require 'open3'

command_re = /^\s\s\s\s(\w+)/
plugin_file = 'plugin/consul.vim'

# Create the list of commands.
stdout, stderr, _status = Open3.capture3('consul list-commands')
output = if stderr == ''
           stdout.split("\n")
         else
           stderr.split("\n")
